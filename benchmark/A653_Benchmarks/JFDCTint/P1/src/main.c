 ...
 *
 *
 * Test:  JPEG slow-but-accurate integer implementation of the forward  DCT
...
void* dct_thread(void)
{
    ...
    {
        OUTPUT("[DCT] Cannot get partition status [%d]\n", ret_type);
        return (void*)1;
    }

    while(1)
    {
        OUTPUT("Computing DCT\n");
        ...
        if(ret_type != NO_ERROR)
        {
            OUTPUT("[DCT] Cannot achieve periodic wait [%d]\n", ret_type);
            return (void*)1;
        }
    }
...
int main()
{
    PROCESS_ID_TYPE        thread_dct;
    PROCESS_ATTRIBUTE_TYPE th_attr_dct;

    RETURN_CODE_TYPE       ret_type;


    /* Set DCT manipulation process */
    th_attr_dct.ENTRY_POINT   = dct_thread;
    th_attr_dct.DEADLINE      = HARD;
    th_attr_dct.PERIOD        = 100000000;
    th_attr_dct.STACK_SIZE    = 1000;
    th_attr_dct.TIME_CAPACITY = 100000000;
    th_attr_dct.BASE_PRIORITY = 1;
    memcpy(th_attr_dct.NAME, "DCT_A653\0", 9 * sizeof(char));

    OUTPUT("Init P0 partition\n");

    /* Create processes */
    CREATE_PROCESS(&th_attr_dct, &thread_dct, &ret_type);
    if(ret_type != NO_ERROR)
    {
        OUTPUT("Cannot create DCT process [%d]\n", ret_type);
        return -1;
    }

    /* Start all processes */
    START(thread_dct, &ret_type);
    if(ret_type != NO_ERROR)
    {
        OUTPUT("Cannot start DCT process[%d]\n", ret_type);
        return -1;
    }
