...
void* ludcmp_thread(void)
{
    ...
    {
        printf("[LUDCMP] Cannot get partition status [%d]\n", ret_type);
        return (void*)1;
    }

    while(1)
    {
        printf("Solving %u equations\n", EQU_COUNT);
        ...
        if(ret_type != NO_ERROR)
        {
            printf("[LUDCMP] Cannot achieve periodic wait [%d]\n", ret_type);
            return (void*)1;
        }
    }
...
int main()
{
    ...
    if(ret_type != NO_ERROR)
    {
        printf("Cannot create LUDCMP process [%d]\n", ret_type);
        return -1;
    }
    ...
    if(ret_type != NO_ERROR)
    {
        printf("Cannot start LUDCMP process[%d]\n", ret_type);
        return -1;
    }
