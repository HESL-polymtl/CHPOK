...
void* fir_thread(void)
{
    ...
    while(1)
    {
        ...
        for(int j = 0; j < SAMPLE_SIZE; ++j)
        {
            ...
            /* first with filter */
            for(i = 0 ; i < 10 ; i++) {
                x[j] = sin(0.05*2*PI*i) + sigma*gaussian();
                x[j] *= 25000.0;         /* scale for D/A converter */
                fir_filter(x[j],fir_lpf35,35,hist);
            }
            /* now without filter */
            for(i = 0 ; i < 10 ; i++) {
                x[j] = sin(0.05*2*PI*i) + sigma*gaussian();
...
int main()
{
    ...
    th_attr_fir.PERIOD        = 200000000;
    th_attr_fir.STACK_SIZE    = 4096;
    th_attr_fir.TIME_CAPACITY = 200000000;
