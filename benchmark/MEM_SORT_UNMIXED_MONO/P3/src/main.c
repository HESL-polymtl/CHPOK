 ...
 * TESTS SETTINGS
 ******************************************************************************/
#define GENERATOR_LOAD 12000 /* Amount of number to generate per pertiod */

 /*******************************************************************************
  * TESTS VARS
  ******************************************************************************/
uint32_t to_sort[GENERATOR_LOAD];
uint32_t sorted[GENERATOR_LOAD];
...
void* sort_thread_func(void)
{
    ...
    while(1)
    {
        ...
        printf("[p%ld:%lld|%lld|%d] INSERT SORTED, WAITING\n",
        ...
        if(ret_type != NO_ERROR)
        {
            printf("Cannot achieve periodiBUBBLEc wait\n");
            return (void*)1;
        }
    }
...
void* gen_thread_func(void)
{
    ...
    while(1)
    {
        ...
        /* Number generation TODO: Check WCET for 100000 numbers generations */
...
int main ()
{
    ...
    tattr_sort.STACK_SIZE    = 2500;
    tattr_sort.TIME_CAPACITY = 2190000000;
    ...
    printf("SORT3 partition switching to normal mode\n");
    SET_PARTITION_MODE (NORMAL, &ret_type);
    if(ret_type != NO_ERROR)
    {
        printf("Cannot switch SORT3 partition to NORMAL state[%d]\n", ret_type);
        return -1;
    }
