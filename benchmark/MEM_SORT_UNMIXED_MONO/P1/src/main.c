...
int main ()
{
    ...
    tattr_sort.TIME_CAPACITY = 1290000000;
    ...
    printf("SORT1 partition switching to normal mode\n");
    SET_PARTITION_MODE (NORMAL, &ret_type);
    if(ret_type != NO_ERROR)
    {
        printf("Cannot switch SORT1 partition to NORMAL state[%d]\n", ret_type);
        return -1;
    }
